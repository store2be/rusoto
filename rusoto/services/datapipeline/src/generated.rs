...
pub struct InstanceIdentity {
    ...
    #[doc="<p>A signature which can be used to verify the accuracy and authenticity of the information provided in the instance identity document.</p>"]
...
pub struct QueryObjectsInput {
    ...
    #[doc="<p>Indicates whether the query applies to components or instances. The possible values are: <code>COMPONENT</code>, <code>INSTANCE</code>, and <code>ATTEMPT</code>.</p>"]
    #[serde(rename="sphere")]
    pub sphere: String,
}
    ...
    pub fn from_body(body: &str) -> GetPipelineDefinitionError {
        match from_str::<SerdeJsonValue>(body) {
            Ok(json) => {
                ...
                match *error_type {
                    "InternalServiceError" => GetPipelineDefinitionError::InternalServiceError(String::from(error_message)),
                    "InvalidRequestException" => {
    ...
    pub fn from_body(body: &str) -> PutPipelineDefinitionError {
        match from_str::<SerdeJsonValue>(body) {
            Ok(json) => {
                ...
                match *error_type {
                    "InternalServiceError" => PutPipelineDefinitionError::InternalServiceError(String::from(error_message)),
                    "InvalidRequestException" => {
    ...
    pub fn from_body(body: &str) -> ReportTaskRunnerHeartbeatError {
        match from_str::<SerdeJsonValue>(body) {
            Ok(json) => {
                ...
                match *error_type {
                    "InternalServiceError" => ReportTaskRunnerHeartbeatError::InternalServiceError(String::from(error_message)),
                    "InvalidRequestException" => {
    ...
    pub fn from_body(body: &str) -> ValidatePipelineDefinitionError {
        match from_str::<SerdeJsonValue>(body) {
            Ok(json) => {
                ...
                match *error_type {
                    "InternalServiceError" => ValidatePipelineDefinitionError::InternalServiceError(String::from(error_message)),
                    "InvalidRequestException" => {
                        ValidatePipelineDefinitionError::InvalidRequest(String::from(error_message))
                    }
                    "PipelineDeletedException" => ValidatePipelineDefinitionError::PipelineDeleted(String::from(error_message)),
                    "PipelineNotFoundException" => ValidatePipelineDefinitionError::PipelineNotFound(String::from(error_message)),
                    "ValidationException" => {
...
pub trait DataPipeline {
    #[doc="<p>Validates the specified pipeline and starts processing pipeline tasks. If the pipeline does not pass validation, activation fails.</p> <p>If you need to pause the pipeline to investigate an issue with a component, such as a data source or script, call <a>DeactivatePipeline</a>.</p> <p>To activate a finished pipeline, modify the end date for the pipeline and then activate it.</p>"]
    ...
    #[doc="<p>Creates a new, empty pipeline. Use <a>PutPipelineDefinition</a> to populate the pipeline.</p>"]
    fn create_pipeline(&self,
                       input: &CreatePipelineInput)
                       -> Result<CreatePipelineOutput, CreatePipelineError>;


    #[doc="<p>Deactivates the specified running pipeline. The pipeline is set to the <code>DEACTIVATING</code> state until the deactivation process completes.</p> <p>To resume a deactivated pipeline, use <a>ActivatePipeline</a>. By default, the pipeline resumes from the last completed execution. Optionally, you can specify the date and time to resume the pipeline.</p>"]
    ...
    #[doc="<p>Gets the object definitions for a set of objects associated with the pipeline. Object definitions are composed of a set of fields that define the properties of the object.</p>"]
    ...
    #[doc="<p>Task runners call <code>EvaluateExpression</code> to evaluate a string in the context of the specified object. For example, a task runner can evaluate SQL queries stored in Amazon S3.</p>"]
    fn evaluate_expression(&self,
                           input: &EvaluateExpressionInput)
                           -> Result<EvaluateExpressionOutput, EvaluateExpressionError>;


    #[doc="<p>Gets the definition of the specified pipeline. You can call <code>GetPipelineDefinition</code> to retrieve the pipeline definition that you provided using <a>PutPipelineDefinition</a>.</p>"]
    ...
    #[doc="<p>Queries the specified pipeline for the names of objects that match the specified set of conditions.</p>"]
    ...
    #[doc="<p>Task runners call <code>SetTaskStatus</code> to notify AWS Data Pipeline that a task is completed and provide information about the final status. A task runner makes this call regardless of whether the task was sucessful. A task runner does not need to call <code>SetTaskStatus</code> for tasks that are canceled by the web service during a call to <a>ReportTaskProgress</a>.</p>"]
    fn set_task_status(&self,
                       input: &SetTaskStatusInput)
                       -> Result<SetTaskStatusOutput, SetTaskStatusError>;


    #[doc="<p>Validates the specified pipeline definition to ensure that it is well formed and can be run without error.</p>"]
...
pub struct DataPipelineClient<P, D>
